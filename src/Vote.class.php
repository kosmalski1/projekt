...
class Vote {
    ...
    */
    ...
    public static function upVote(int $postId, int $userId) : bool {
        ...
        global $db;
        $query = $db->prepare("INSERT INTO rating VALUES (NULL, ?, 1, ?)");
    ...
    public static function getScore(int $postId) : int {
        ...
        $query = $db->prepare("SELECT SUM(score) FROM rating WHERE post_id = ?");
    ...
    public static function getVote(int $postId, int $userId) : int {
        ...
        global $db;
        $query = $db->prepare("SELECT score FROM rating WHERE post_id = ? AND user_id = ?");
        $query->bind_param('ii', $postId, $userId);
        if($query->execute()) {
            $result = $query->get_result();
            if($result->num_rows > 0)
